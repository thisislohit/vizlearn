...
import 'package:vizlearn/views/widgets/multi_select_dropdown/core/search_multiselect_field.dart';

import '../../../../gen/assets.gen.dart';
import 'chip_multiselect_field.dart';
...
class _MultiSelectFieldState<T> extends State<MultiSelectField<T>> with AutomaticKeepAliveClientMixin {
  final bool _isMobile = (defaultTargetPlatform == TargetPlatform.iOS || defaultTargetPlatform == TargetPlatform.android);
  ...
  List<Choice<T>> get _cleanData => widget.data().where((ele) => ele.key != null || ele.key!.isNotEmpty).toList();

  ItemColor get _currentItemColor => ItemColor(selected: Colors.blueAccent.shade100.withAlpha(50), unSelected: Colors.transparent, hovered: Colors.blueAccent.shade100.withAlpha(30));
  ...
  void didUpdateWidget(covariant MultiSelectField<T> oldWidget) {
    if (widget.defaultData != null && widget.defaultData!.isNotEmpty && widget.singleSelection) {
  ...
  Widget build(BuildContext context) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((obs) {
      ...
      if (_menuController.isOpen && isKeyboardOpen) {
        final int visibleItems = _onFilteredChoice.length;
        const double itemHeight = 48; // approx height of each MenuItemButton
        const double spacing = 5; // padding or gap
        final double calculatedHeight = (visibleItems * itemHeight) + spacing;

        // Limit to max height (e.g. max 5 items)
        final double dynamicOffset = -(calculatedHeight.clamp(0, 210.0) as double);

        _menuController.open(position: Offset(0, dynamicOffset));
            ...
            return MenuAnchor(
              ...
              builder: (context, menu, child) {
                ...
                currentMenuHeight = widget.menuHeightBaseOnContent
                    ...
                    : widget.menuHeight ?? 300; // Otherwise, use a predefined height or default to 300.
                return InkWell(
                  ...
                  child: KeyboardListener(
                    ...
                    onKeyEvent: (event) {
                      ...
                      if (event.runtimeType == KeyDownEvent) {
                        ...
                        if (event.logicalKey == LogicalKeyboardKey.backspace) {
                          if (_selectedChoice.isNotEmpty && _textController.text.isEmpty) {
                            _addOrRemove(_selectedChoice[_selectedChoice.length - 1]);
                          }
                        }
                        ...
                        if (event.logicalKey == LogicalKeyboardKey.enter) {
                          if (_textController.text.isNotEmpty) {
                            ...
                            Choice<T> elementFiltered = widget.data().firstWhere(
                                  (filter) => filter.value.toLowerCase() == _textController.text.toLowerCase(),
                                );
                    ...
                    child: Container(
                      ...
                      child: Flex(
                        ...
                        children: [
                          if (widget.iconLeft != null) widget.iconLeft!(_menuController.isOpen),
                          Flexible(
                            fit: FlexFit.loose,
                            child: Wrap(
                              direction: Axis.horizontal,
                              ...
                              children: [
                                if (_selectedChoice.isEmpty && !widget.useTextFilter && widget.label != null)
                                  Text(
                                    widget.label!,
                                      ...
                                      if (!widget.singleSelection) {
                                        return widget.multiSelectWidget != null
                                            ...
                                            : ChipMultiselectField(
                                                title: element.value,
                                                onDeleted: () => _addOrRemove(element),
                                        ...
                                        return widget.singleSelectWidget != null
                                            ...
                                            : Text(
                                                element.value,
                                                style: widget.textStyleSingleSelection ?? Theme.of(context).textTheme.labelLarge,
                                              );
                                      }
                                    },
                                  ),
                                if ((widget.useTextFilter && _selectedChoice.isEmpty && widget.singleSelection) || (widget.useTextFilter && !widget.singleSelection))
                                  ...
                                  child: Center(
                                    child: GestureDetector(
                                      child: CustomImageView(
                                        imagePath: menu.isOpen ? Assets.icons.arrowUp : Assets.icons.arrowDown,
              ...
              menuChildren: [
                ...
                ..._onFilteredChoice.where((element) => element.value.isNotEmpty).map(
                  (result) {
                    ...
                    return SizedBox(
                      ...
                      child: Column(
                        children: [
                          MenuItemButton(
                            closeOnActivate: widget.singleSelection || widget.data().length == 1,
                            key: (!isGroupingTitle && _isSelected(result)) && _selectedChoice.indexOf(result) == 0 ? _selectedItemKey : null,
                            ...
                            style: widget.buttonStyle ??
                                ButtonStyle(
                                  ...
                                  backgroundColor: WidgetStateProperty.resolveWith((state) {
                                    ...
                                    if ((state.contains(WidgetState.hovered) && _isMobile)) {
                                      return widget.itemColor?.hovered ?? _currentItemColor.hovered;
                                    }

                                    /// Color When is element selected
                                    if ((!isGroupingTitle && _isSelected(result))) {
                                      return widget.itemColor?.selected ?? _currentItemColor.selected;
                                    }

                                    /// Default color, No selected, no hovered.
                                    return widget.itemColor?.unSelected ?? _currentItemColor.unSelected;
                                  }),
                                ),
                            onPressed: isGroupingTitle
                                ...
                                : () {
                                    _addOrRemove(result);
                                    if (!widget.singleSelection && widget.useTextFilter) {
                                      _textController.clear();
                                    }
                                  },
                            child: widget.itemMenuButton ??
                                Padding(
                                  ...
                                  child: Text(result.value, style: isGroupingTitle ? widget.titleMenuStyle ?? Theme.of(context).textTheme.titleLarge : widget.itemMenuStyle ?? Theme.of(context).textTheme.labelLarge),
                                ),
                          ),
                          if (!(widget.data().last == result))
                            Container(
                              height: 1,
                              color: Colors.grey,
              ...
              style: widget.menuStyle ??
                  MenuStyle(
                    ...
                    maximumSize: widget.menuWidthBaseOnContent && widget.menuHeightBaseOnContent
                        ? null
  ...
  void _addOrRemove(Choice<T>? va) {
    if (va != null) {
      if (widget.singleSelection) {
        ...
        if (_selectedChoice.isNotEmpty && _isSelected(va) && !widget.isMandatory) {
          _selectedChoice.clear();
          _textController.clear();
        } else {
...
class Choice<T> {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is Choice && runtimeType == other.runtimeType && key == other.key && value == other.value && metadata == other.metadata;
